...
const results = input.map((calibration): number | boolean => {
  ...
  for (let num = 0; num < Math.pow(3, calibration.values.length); num++) {
    const combo = num.toString(3).padStart(calibration.values.length, "0");
  ...
  const combinationResults = totalCombinations.map((fullCombination) => {
    ...
    return calibration.values.reduce((previous, current, index) => {
      ...
      if (combination[index] === "2") return Number(`${previous}${current}`);
